...
class CredentialContract extends Contract {
  ...
  async _isValidIssuer(ctx) {
    ...
    const key = ctx.stub.createCompositeKey("USER", [mspId, "issuer", userId]);
  ...
  async GetCredential(ctx, vcId) {
    ...
    if (credential.issuer !== callerMSP) {
      throw new Error("Access denied: cross-org credential access not allowed");
    }
  ...
  async IssueCredential(ctx, vcId, cid, holder, issuedAt) {
    ...
    await ctx.stub.putState(
      ctx.stub.createCompositeKey("CRED_ISSUER", [issuerInfo.userId, vcId]),
      Buffer.from("\u0000"),
    );
  ...
  async RevokeCredential(ctx, vcId, revokedAt) {
    ...
    if (credential.issuerId !== issuer.userId) {
      throw new Error(`Only the issuing entity can revoke this credential`);
    }
